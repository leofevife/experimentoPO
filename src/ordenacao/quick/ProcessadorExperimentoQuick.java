...
public class ProcessadorExperimentoQuick {

    private final String[] arquivosDados;

    public ProcessadorExperimentoQuick(String[] arquivosDados) {
    ...
    public void executar() {
        ...
        System.out.println("Processamento Quick Sort concluído.");
    }

    private void processarArquivoUnico(String arquivoDado) {
        // Define nome de saída: ex "Quick1000alea.txt"
        String arquivoSaida = "Quick" + arquivoDado.substring(7);
        System.out.println("Processando Quick Sort: " + arquivoDado + "...");

        try {
            long inicioTotal = System.nanoTime();

            //Executa 5 vezes para calcular a média
            for (int i = 0; i < 5; i++) {
                // Carrega os dados
    ...
    private void gravarDadosOrdenados(ArrayList<Reserva> lista, String arquivoSaida) throws IOException {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(arquivoSaida))) {
            for (Reserva r : lista) {
                // Formato de impressão da lista completa
